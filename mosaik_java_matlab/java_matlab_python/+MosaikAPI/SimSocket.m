classdef SimSocket < handle
    ...
    methods
        ...
        function this = SimSocket(server,port,varargin)
            ...
            this.server
            this.port
            this.socket = tcpip(this.server,this.port,'NetworkRole','server');
            fopen(this.socket);
            a=2
    ...
    methods (Access=private)
        ...
        function messages = readSocket(this)
            ...
            a=4
            % Read the socket
            buffer = fread(this.socket,this.socket.BytesAvailable)
            a=5
        ...
        function messages = splitRequest(this,buffer)
            ...
            char(buffer)
            sum(uint32(buffer(1:4)).*uint32([4 3 2 1].^8))
            messages = {};
            
            while numel(buffer)>0
                
                try
                    messageLength = sum(uint32(buffer(1:4)));
                catch ME
