...
class Fake_Proxy():
    ...
    def __init__(self, name, sid, meta, proc, rpc_con, world):
        ...
        self.step_required = None  # SimPy

    def stop(self):
        self._world.client.publish(self.sid+"/in/stop","")
    ...
    *ExampleSimC* can not be started by mosaik, so mosaik tries to connect to
    ...
    for sim_type, start in starters.items():
        if sim_type in sim_config:
            if sim_type == "connect":
                world.client.publish(world.commands["control"][sim_name]["port"]+"/init",str([sim_id,str(sim_params)]))
                world.primary_sem.acquire()
                meta = world.commands["init"][sim_id]
                return Fake_Proxy(sim_name,sim_id,meta,None,None,world)
            else:
