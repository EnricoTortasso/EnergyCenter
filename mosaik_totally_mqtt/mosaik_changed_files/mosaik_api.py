...
def on_message(client, userdata, message):
    ...
    if path[1]=="in":
        if path[2] == "step":
            inputs= ast.literal_eval(str(message.payload.decode("utf-8")))
            next_time = client.sim.step(inputs[0],inputs[1])###inputs[0]+ 60 ##
            client.publish(path[0]+"/out/step",str(next_time))
        elif path[2]=="get_sync_data":
            inputs = ast.literal_eval(str(message.payload.decode("utf-8")))
            data = client.sim.get_data(inputs)  ###inputs[0]+ 60 ##
            client.publish(path[0] + "/out/get_sync_data", str(data))
        elif path[2] == "create":
            inputs = ast.literal_eval(str(message.payload.decode("utf-8")))
            data = client.sim.create(inputs[0],inputs[1])
            client.publish(path[0] + "/out/create", str(data))
        elif path[2] == "stop":
            client.sim.sem.release()
    elif path[1]=="init":
        inputs = ast.literal_eval(str(message.payload.decode("utf-8")))
        meta = client.sim.init(inputs[0])
        client.subscribe(inputs[0]+"/in/#")
        client.publish(inputs[0] + "/out/init", str(meta))

    #print("Hi "+ client._client_id.decode("utf-8"))
...
class Simulator(object):
    ...
    *extra_methods* is an optional list of methods that a simulator provides in
    ...
    def init_plus(self,sid,**sim_params):
        return
...
def start_simulation(simulator, description='', extra_options=None):
    ...
    *extra_option* may be a list of options for `docopt <http://docopt.org/>`_
    ...
    try:
        ...
        simulator.client.subscribe(":".join([addr[0],str(addr[1])])+"/init")
        simulator.sem.acquire()
...
def run(channel, sim):
    """Main simulator process. Send a greating message to mosaik and wait       #qui è dove credo bisogna inserire
