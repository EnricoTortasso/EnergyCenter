...
def on_message(client, userdata, message):
    '''gestisce i messaggi in arrivo dall'MQTT broker
    esempio: un sensore pubblica nel topic "<simulatore>/<modello>/<sensore>/<attributo>" il messaggio "valore"
    alla fine client.world.commands = {
                                    "tmp" : {
                                        "<simulatore>" : {
                                            "<modello>" : {
                                                "<sensore>" : {
    ...
    se il messaggio serve a dire che c'è un simulatore in remoto pronto per il collegamento
    allora il messaggio deve essere di tipo:
    topic = "control/<nome_simulatore>"
    messaggio = "<porta_TCP/IP>,<attr1>.<attr2>.<attr3>.....<attrN>,<eid_prefix>,<create_method>,<info_connessioni>"
    dove <info_connessioni> = <full_id_1_1>%<attr_con1>%<attr_con2>%<attr_con3>;...;<full_id_n_1>/<full_id_1_2>;...;<full_id_n_2>/.../<full_id_1_n>;...;<full_id_n_n>
    dove <attr_con> = <attributo_da_mandare> [ > <nome_con_cui_viene_ricevuto>]
    '''
    #print(message.payload)
    path = message.topic.split('/')
    if path[0]=="control":                                              #messaggio per indicare un nuovo simulatore attivo
        if "control" not in client.world.commands:
            client.world.commands["control"]={}
        client.world.commands["control"][path[1]]={}
        infos = str((message.payload).decode("utf-8")).split(",")
        client.world.commands["control"][path[1]]["port"] = infos[0]
        client.world.commands["control"][path[1]]["attrs"] = []
        client.world.commands["control"][path[1]]["prefix"] = infos[2]
        client.world.commands["control"][path[1]]["create"] = infos[3]
        client.world.commands["control"][path[1]]["dests"] = {}
        for attr in infos[1].split("."):
            if attr != "":
                client.world.commands["control"][path[1]]["attrs"].append(attr)
        i=0

        for dest in infos[4].split("/"):
            if dest != "":
                model_name = infos[2]+str(i)
                client.world.commands["control"][path[1]]["dests"][model_name]={}
                for conn_info in dest.split(";"):
                    conns = conn_info.split("%")
                    client.world.commands["control"][path[1]]["dests"][model_name][conns[0]]={}
                    n = len(conns)
                    for m in range(1,n):
                        couple = conns[m].split(">")
                        if len(couple)==2:
                            client.world.commands["control"][path[1]]["dests"][model_name][conns[0]][couple[0]] = \
                            couple[1]
                        elif len(couple)==1:
                            client.world.commands["control"][path[1]]["dests"][model_name][conns[0]][couple[0]] = \
                            couple[0]
        ...
        if path[2] == "step":
            if "step" not in client.world.commands:
                client.world.commands["step"] ={}
            client.world.commands["step"][path[0]] = int(str(message.payload.decode("utf-8")))
            client.world.sems[path[0]].release()
        elif path[2] == "get_sync_data":
            if "get_sync_data" not in client.world.commands:
                client.world.commands["get_sync_data"] = {}
            data = ast.literal_eval(str(message.payload.decode("utf-8")))
            client.world.commands["get_sync_data"][path[0]] = data
            client.world.sems[path[0]].release()
        elif path[2] == "init":
            if "init" not in client.world.commands:
                client.world.commands["init"] = {}
            data = ast.literal_eval(str(message.payload.decode("utf-8")))
            client.world.commands["init"][path[0]] = data
            for key in  client.world.commands["init"][path[0]]["models"].keys():
                if "any_inputs" not in client.world.commands["init"][path[0]]["models"][key]:
                    client.world.commands["init"][path[0]]["models"][key]["any_inputs"]=False
            client.world.primary_sem.release()
        elif path[2] == "create":
            if "create" not in client.world.commands:
                client.world.commands["create"] = {}
            data = ast.literal_eval(str(message.payload.decode("utf-8")))
            client.world.commands["create"][path[0]] = data
            client.world.sems[path[0]].release()
    else:
        if "tmp" not in client.world.commands:
            client.world.commands["tmp"]= {}
        if path[0] not in client.world.commands["tmp"]:
            client.world.commands["tmp"][path[0]] = {}
        if path[1] not in client.world.commands["tmp"][path[0]]:
            client.world.commands["tmp"][path[0]][path[1]] = {}
        if path[2] not in client.world.commands["tmp"][path[0]][path[1]]:
            client.world.commands["tmp"][path[0]][path[1]][path[2]] = {}
        client.world.commands["tmp"][path[0]][path[1]][path[2]][path[3]] = str(message.payload.decode("utf-8"))
...
class World:
    ...
    *start_timeout* and *stop_timeout* specifiy a timeout (in seconds) for
    ...
    def __init__(self, sim_config, mosaik_config=None, debug=False):
        ...
        self.client = client.Client("mondo")
        setattr(self.client, "world", self)
        self.commands={}
        ...
        for nome, comms in sim_config.items():
            for comm, port in comms.items():
                self.commands["control"][nome]={}
                self.commands["control"][nome]["port"] = port
        ...
        self.models = {}                                                            #
        self.sems = {}
        self.primary_sem = Semaphore()
        self.primary_sem.acquire()
        self.procs = {}
    ...
    def start(self, sim_name, **sim_params):
        ...
        n=len(self.modelsFac)                                                      #
        self.modelsFac[sim_id]= ModelFactory(self, sim)                            #
        self.sems[sim_id] = Semaphore()
        self.sems[sim_id].acquire()
        self.client.subscribe(sim_id+"/out")
        self.procs[sim_id]= self.env.event()
        return self.modelsFac[sim_id]
    ...
    def connect(self, src, dest, *attr_pairs, async_requests=False,
        ...
        *initial_data* should contain a dict with input data for the first
        ...
        src.connections[dest.full_id]= attr_pairs
        ...
        if "control" in self.commands:                                      #cerca se ci sono altri simulatori in attesa
            ...
            for sim ,data in control.items():
                ...
                if "prefix" in data:
                    ...
                    eval("factor."+data["create"]+".create("+ str(len(data['dests']))+")")  #bisogna mettere molti controlli prima di questa funzione
                    ...
                    for model,conns in data["dests"].items():
                        ...
                        for conn,couples in conns.items():
                            for a,b in couples.items():
                                tupla = (a,b)
                                self.connect(self.models[string], self.models[conn], tupla, async_requests=True)
    ...
    def _check_attributes(self, src, dest, attr_pairs):
        ...
        """
        entities = [src, dest]
        emeta = [self.commands["init"][e.sid]['models'][e.type] for e in entities]
...
class ModelMock:
    ...
    def __init__(self, world, name, sim):
        ...
        self._sim = sim
        if sim is None:
            self._sim_id = identifier
            self._params = world.commands["init"][identifier]['models'][name]['params']
        else:
    ...
    def create(self, num, **model_params):
        ...
        """
        self._check_params(**model_params)
        if isinstance(self._sim,Fake_Proxy):
            self._world.client.publish(self._sim_id+"/in/create",str([num,self._name,model_params]))
            self._world.sems[self._sim_id].acquire()
            entities = self._world.commands["create"][self._sim_id]
        else:
        ...
        assert len(entities) == num, (
            ...
            (num, len(entities)))
        entity_set = self._make_entities(entities,self._name, assert_type=self._name)
        for entity in entity_set:
            self._world.models[entity.full_id] = entity
        return entity_set
    ...
    def _make_entities(self, entity_dicts,name, assert_type=None):
...
class Entity:
    ...
    __slots__ = ['sid', 'eid', 'sim_name', 'type', 'children', 'sim', 'connections']
