    ...
    public long step(long time, Map<String, Object> inputs) {
        ...
        for (Map.Entry<String, Object> entity : inputs.entrySet()) {
            ...
            for (Map.Entry<String, Object> attr : attrs.entrySet()) {
                ...
                if (attrName.equals("pace")) {
                    if (((Map<String, Object>)attr.getValue()).values().toArray()[0] == null);
                    else {
                    	String val = (String) ((Map<String, Object>)attr.getValue()).values().toArray()[0];
                    	String eid = entity.getKey();
                    	if (val.split(":")[0].equals(eid)) {
                    		int idx = this.entities.get(eid);
                    		this.simulator.set_pace(idx, Integer.parseInt(val.split(":")[1]));
                    	}
    ...
    public Map<String, Object> getData(Map<String, List<String>> outputs) {
        ...
        for (Map.Entry<String, List<String>> entity : outputs.entrySet()) {
            ...
            for (String attr : attrs) {
                ...
                else if (attr.equals("pace")) {
                	values.put(attr, this.simulator.get_pace(idx));
                }
            }
            data.put(eid, values);
        }
