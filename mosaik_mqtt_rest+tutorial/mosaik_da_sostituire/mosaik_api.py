...
class Simulator(object):
    ...
    *extra_methods* is an optional list of methods that a simulator provides in
    ...
    def init(self, sid, **sim_params):
        ...
        """
        mqtt = sim_params.get("mqtt",None)
        rest = sim_params.get("rest",None)
        if mqtt:
            self.mqtt=mqtt
        ...
        if rest:
            self.rest=rest
            self.rest_conn = HTTPConnection(rest["address"])

            self.rest_commands = {}

            for attr in rest["attrs"].keys():
                self.last_update[attr] = datetime.now()
    ...
    def step(self, time, inputs):
        ...
        if self.rest_conn:
            now = datetime.now()
            for attr in self.rest["attrs"].keys():
                delta = (now - self.last_update[attr]).total_seconds()
                if (now - self.last_update[attr]).total_seconds() > int(self.rest["attrs"][attr]["timeout"]):
                    print("sto provando a connettermi")
                    self.rest_conn.request("GET",self.rest["attrs"][attr]["GET"])
                    resp = self.rest_conn.getresponse()
                    self.last_update[attr] = datetime.now()
                    self.rest_commands[attr] = resp.read().decode("utf-8")
                    print(self.rest_commands[attr])

        a = self.step_plus(time,inputs)
        if isinstance(a,int):
            return a
        b,c = a
        yield self.mosaik.set_data(c)
        return b
